...
export class AuthService {
  ...
  async registerStudent(collegeEmail: string, password: string) {
    // Check existing email
    const existingStudent = await this.authRepository.findStudentByEmail(collegeEmail);
  ...
  async verifyOtp(studentId: string, otpCode: string) {
    // Find student
    ...
    // Check OTP expiry
    if (!student.otpExpiresAt || student.otpExpiresAt.getTime() < Date.now()) {
